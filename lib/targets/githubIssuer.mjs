...
const log = getLogger("targets/githubIssuer");
...
export default class Issuer {
    ...
    static async discover(url) {
        log.info({info: "discover issuer", url});

        if (!(url && url.length)) {
            log.debug("empty URL provided");
        ...
        if (response.status === 404) {
            log.debug({info: "URL is not a github instance", url});
...
class GHOAuthClient {
    ...
    async userinfo(access_token) {
        ...
        const response = await fetch(this.issuer.userUrl, {
            signal,
            headers,
            cache
        });
        ...
        const userData = Object.keys(userInfo).reduce((t,k) => {
            if (keepUserFields.includes(k)) {
                t[k] = userInfo[k];
            }
    ...
    async revoke(access_token) {
        ...
        headers.set("Authorization", `Basic ${Buffer.from(this.client_id + ":" + this.#client_secret).toString("base64")}`);
        ...
        log.debug({info: "revocation result", result});
