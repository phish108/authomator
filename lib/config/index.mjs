...
export default async function Config(file, defaults) {
    ...
    if (cfgFile === undefined) {
        console.log(`${JSON.stringify({message: "No config files found"})}`);
    ...
    if (!result) {
        console.log(`${JSON.stringify({message: "Empty config"})}`);
        result = {};
    }
    
    if (!("backend" in result && result.backend)) {
        console.log(`${JSON.stringify({message: "no backend defined. use the current config."})}`);
    ...
    if (!("type" in result.backend)) {
        console.log(`${JSON.stringify({message: "backend without type"})}`);
    ...
    if (result.backend.type.toLowerCase() !== "file") {        
        try {
            ...
            console.log(`${JSON.stringify({message: `Configtype ${result.backend.type} does not exist, use file backend`})}`);
